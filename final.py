...
def send_reset_email(recipient_email, token):
    reset_url = f"https://financeapp-new.streamlit.app//?reset_token={token}"
...
def clean_orphaned_log_entries():
    """Remove log entries that don't have corresponding users in the database."""
    if not os.path.exists(LOG_FILE):
        return False
    
    # Get all existing usernames from database
    existing_users = set()
    for user in collection.find({}, {"name": 1}):  # Projection to only get the name field
        if 'name' in user:  # Check if name exists in the document
            existing_users.add(user['name'])
    
    # Read log file
    df = pd.read_excel(LOG_FILE)
    
    # Filter out rows where name doesn't exist in database
    original_count = len(df)
    df = df[df['name'].isin(existing_users)].reset_index(drop=True)
    new_count = len(df)
...
def delete_user(user_name):
    """Delete a user completely from the system including all their actions in the log.
    Also updates total user count."""
    # First check if user exists in database
    user = collection.find_one({"name": user_name})
    if not user:
        return False
    
    # Delete all log entries for this user
    if os.path.exists(LOG_FILE):
        df = pd.read_excel(LOG_FILE)
        df = df[df['name'] != user_name].reset_index(drop=True)
        df.to_excel(LOG_FILE, index=False)
    ...
    # Decrement total user count in stats
    collection.update_one(
        {"_id": "dashboard_stats"},
        {"$inc": {"total_users": -1}},
        upsert=True
...
if selected == "Add User":
    st.markdown(create_dashboard_header("👤 User Management"), unsafe_allow_html=True)
    
    tab1, tab2 = st.tabs(["Add New User", "Delete Existing User"])
        ...
        with st.form("add_loan"):
            ...
            if st.form_submit_button("Add User"):
                if not name.strip():
                    ...
                    st.error(f"User '{name}' already exists in your records!")
                else:
    ...
    with tab2:
        user_filter = get_user_filter()
        existing_users = [u['name'] for u in collection.find(user_filter, {"name": 1})]
        
        if not existing_users:
            st.warning("No users found to delete.")
        else:
            user_to_delete = st.selectbox("Select user to delete", existing_users)
            
            if st.button("Delete User", type="primary"):
                with st.spinner("Deleting user and all associated records..."):
                    if delete_user(user_to_delete):
                        st.success(f"User '{user_to_delete}' and all their records have been deleted!")
                        st.rerun()
                    else:
